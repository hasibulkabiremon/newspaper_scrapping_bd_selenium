...
def main():
    ...
    category_driver.get("https://www.ittefaq.com.bd/")

    category_elements = category_driver.find_elements(By.XPATH, "//div[@id='main_menu']/ul/li/a[contains(@class,'static') or contains(@class,'dynamic')]")
...
def headline(headlineLink):
    ...
    print(headlineLink)
    headline_element = headline_driver.find_elements(By.XPATH,"//a[@class='link_overlay']")
    
    headlines = [{'text': h.get_attribute('title'), 'link': h.get_attribute('href')} for h in headline_element]
...
def news_Details(newsLink):
    ...
    news_author = news_driver.find_element(By.XPATH, "//*[@class='name']").text
    news_time = news_driver.find_element(By.XPATH,"//*[@class='tts_time']").text
    news_text = news_driver.find_element(By.XPATH,"//*[@class='viewport jw_article_body']").text
